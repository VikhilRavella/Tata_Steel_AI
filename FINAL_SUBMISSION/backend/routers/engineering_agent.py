            ...
            if has_doc and intent in ["PDF", "SAFETY", "REPORT"]:
                ...
                if doc and os.path.exists(doc.file_path):
                    ...
                    if selected_mode == "DOCUMENT_SUMMARY":
                        ...
                        yield f"data: {json.dumps({'status': '🔎 Searching pages...'})}\n\n"
...
4. DO NOT use mandatory output templates like "Answer:", "Source:", "Section:", or "Heading:" unless explicitly requested. Allow the best format to emerge naturally.
5. ONLY show page references and metadata if the user explicitly asks for them using phrases like "where is this mentioned", "source", "page number", "citation", or "show evidence". Otherwise, do not display source metadata.
6. MUST NOT HALLUCINATE. Do not invent facts about inventory or work orders. If you cannot find requested database records, just say so politely.

EXAMPLES OF DYNAMIC STRUCTURE:
- If user asks "What component is visible?": Provide Component, Visible Defects, Confidence.
- If user asks "What maintenance actions do you recommend?": Provide Findings, Recommended Actions, Priority.
